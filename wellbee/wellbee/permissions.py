...
class MembershipPermission(BasePermission):
    def has_permission(self, request, view):
        ...
        if view.action  == 'fetch_available_membership' or view.action == 'fetch_my_all_membership' or view.action == 'fetch_membership_by_staff':
            return (request.user and request.user.is_authenticated) or request.user.is_staff==1
...
class AttendeePermission(BasePermission):
    def has_permission(self, request, view):
        ...
        if view.action  == 'fetch_attendee_by_staff' or view.action == 'fetch_my_attendee':
            return (request.user and request.user.is_authenticated) or request.user.is_staff==1
         # 更新と削除はスタッフのみ
...
class SlotPermission(BasePermission):
    def has_permission(self, request, view):
        ...
        # アクションがmy_reservationだった場合
        if view.action  == 'fetch_course_slots' or view.action == 'fetch_slots_for_staff' or view.action == 'fetch_course_slots_for_staff' or view.action == 'fetch_slots_for_calendar' or view.action =='update_slot':
            return  (request.user and request.user.is_authenticated) or request.user.is_staff == 1
        if view.action in ['create','partial_update','update', 'destroy',]:
            return request.user.is_staff==1
        # デフォルトではアクセスを拒否
