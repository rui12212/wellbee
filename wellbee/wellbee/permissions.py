...
class MembershipPermission(BasePermission):
    def has_permission(self, request, view):
        ...
        if view.action  == 'fetch_available_membership' or view.action == 'fetch_my_all_membership' or view.action == 'fetch_membership_by_staff' or view.action == 'fetch_closest_membership' or view.action == 'fetch_course_membership' or view.action == 'fetch_all_available_membership':
            return (request.user and request.user.is_authenticated) or request.user.is_staff==1
