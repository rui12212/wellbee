...
class NotificationDaysSettingViewSet(viewsets.ModelViewSet):
    """通知日数設定（スタッフ専用）"""
    queryset = NotificationDaysSetting.objects.all()
    serializer_class = serializers.NotificationDaysSettingSerializer
    permission_classes = [IsStaffUser]

    def get_queryset(self):
        if self.action == 'list':
            return NotificationDaysSetting.objects.filter(is_active=True).order_by('days_before_expire')
        return super().get_queryset()

    @action(detail=True, methods=['get'], url_path='target-users')
    def target_users(self, request, pk=None):
        ...
        now_local = now_utc.astimezone(local_timezone)
        today_date = now_local.date()
        target_date = today_date + timedelta(days=setting.days_before_expire)
        ...
        for m in memberships:
            if m.user_id not in seen_user_ids:
                ...
                target_users.append({
                    ...
                    'attendee_name': m.attendee.name if m.attendee else '',
                    'membership_id': m.id,
                    'course_name': m.course.course_name if m.course else '',
                    'expire_day': m.expire_day,
                })

        return Response({
            'setting_id': setting.id,
            'days_before_expire': setting.days_before_expire,
    ...
    def send(self, request, pk=None):
        ...
        unique_users = {}
        for m in memberships:
            if m.user_id and m.user_id not in unique_users:
                unique_users[m.user_id] = m.user
        ...
        try:
            from attendances.utils import send_push_notification
            send_push_notification(
                user_ids=list(unique_users.keys()),
                title='Membership Expiry Notice',
                body=setting.message[:100]
            )
        ...
        return Response({
            ...
            'message': f'Message sent to {len(unique_users)} users.'
        }, status=status.HTTP_200_OK)
...
class UserMessageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return UserMessage.objects.filter(user=self.request.user).order_by('-created_at')
...
class UserFCMTokenViewSet(viewsets.ViewSet):
    """FCMトークン管理"""
    permission_classes = [MembershipPermission]
    ...
    def unregister(self, request):
        token = request.data.get('token')
        if token:
            UserFCMToken.objects.filter(user=request.user, token=token).update(is_active=False)
        else:
            UserFCMToken.objects.filter(user=request.user).update(is_active=False)
        return Response({
            'success': True,
