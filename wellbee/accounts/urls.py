...
from .views import (
    ...
    PasswordResetVerifyFirebaseView,
...
urlpatterns = [
    ...
    path('password-reset/verify-firebase/', PasswordResetVerifyFirebaseView.as_view(), name='password_reset_verify_firebase'),

    # パスワードリセット（スタッフ手動・JWT認証必須）
    path('staff/password-reset/', StaffPasswordResetView.as_view(), name='staff_password_reset'),

    # スタッフユーザー作成（JWT認証必須）
    path('staff/create/', StaffCreateUserView.as_view(), name='staff_create_user'),
